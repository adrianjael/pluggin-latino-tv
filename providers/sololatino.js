 ...
 * Generado: 2026-04-27T21:09:23.967Z
 */
  ...
  "src/sololatino/extractor.js"(exports2, module2) {
    ...
    function getStreams2(tmdbId, mediaType, season, episode) {
      return __async(this, null, function* () {
        var _a;
        try {
          console.log(`[SoloLatino] DirectOrigin v2.7.6: ${mediaType} ID:${tmdbId}`);
          let imdbId = tmdbId;
          ...
          for (const srv of latServers) {
            try {
              ...
              if (videoUrl.includes("/api/source/")) {
                ...
                if (videoUrl.includes("minochinos") || videoUrl.includes("cloudwindow") || serverName.includes("mino") || serverName.includes("vip")) {
                  if (videoUrl.includes("p.php")) {
                    try {
                      const urlParams = new URL(`https://dummy.com${videoUrl}`).searchParams;
                  ...
                  const originDomain = new URL(videoUrl).origin;
                  finalHeaders = {
                    "User-Agent": NUVIO_UA,
                    "Referer": originDomain + "/",
                    // Ej: https://minochinos.com/
                    "origin": originDomain,
                    "Accept": "*/*"
                  };
                } else {
                  ...
                  const masterRes = yield fetch(proxyUrl, { headers: finalHeaders });
                  if (masterRes.ok) {
                    ...
                    if (bestSubM3u8Url) {
                      const subRes = yield fetch(bestSubM3u8Url, { headers: finalHeaders });
                      if (subRes.ok) {
